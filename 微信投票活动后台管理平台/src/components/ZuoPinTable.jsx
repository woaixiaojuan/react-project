...
export default class ZuoPinTable extends React.Component {
  ...
  editDone(record, flag) {
    if (flag === 'save') {
      if (this.props.data.length >= parseInt(this.props.cacheValue, 10) && parseInt(this.props.cacheValue, 10) !== parseInt(record.orderId, 10)) {
        this.props.dispatch(editIndexSave(this.props.data, parseInt(record.optionId, 10), parseInt(record.orderId, 10), parseInt(this.props.cacheValue, 10), record.contentType));
  ...
  render() {
    const { data, tabkey, mark } = this.props;
    ...
    return (
      <div style={{ minHeight: '130px' }}>
        <Table columns={columns} dataSource={data} onChange={this.handleChange.bind(this)} rowKey={'optionId'} />
      </div>
    );
  }
}
