...
export function getZuoPinItem(activeId) {
  return function (dispatch) {
    ...
    .then(axios.spread((zuoPinList, sourceList) => {
      let data = [];
      if (zuoPinList.data.success === true) {
        data = zuoPinList.data.data.map(ele => {
        ...
        openNotification('error', zuoPinList.data.msg);
      }

      if (sourceList.data.success === true) {
        dispatch({
...
function getSortArr(data, optionId, cacheValue, orderId, contentType, type) {
  ...
  if (orderId > cacheValue) {
    arr = data
      .filter(ele => {
        ele.orderId = parseInt(ele.orderId, 10);
        return ele.orderId >= cacheValue && ele.orderId <= orderId && ele.contentType === contentType;
      })
      .map((ele) => {
        ele.orderId = parseInt(ele.orderId, 10);
        const item = {
...
export function editIndexSave(data, optionId, orderId, cacheValue, contentType, type = 0) {
  return function (dispatch) {
    ...
    //   },
    // });
    axios.post(`${ENV.api}/wx/voteActive/option/update.do`, qs.stringify({ type: 2, json: JSON.stringify(jsonArr) }),
    )
    .then((res) => {
...
export function addZuoPinItem(values) {
  return function (dispatch) {
    ...
    //   },
    // });
    axios.post(`${ENV.api}/wx/voteActive/option/add.do`, qs.stringify({ json: JSON.stringify(values) }),
    )
    .then((res) => {
      if (res.data.success === true) {
        dispatch({
...
export function uploadZuoPinItem(values) {
  return function (dispatch) {
    ...
    axios.post(`${ENV.api}/wx/voteActive/option/update.do`, qs.stringify({ type: 1, json: JSON.stringify(values) })
    )
    .then((res) => {
      if (res.data.success === true) {
        dispatch({
...
export function delZuoPinItem(record, recordArr) {
  return function (dispatch) {
    ...
    //   },
    // });
    axios.post(`${ENV.api}/wx/voteActive/option/delete.do`, qs.stringify({
      activeId,
      optionId,
    }),
    )
